...
var pageaction = function(){
    $.get('/api/parameter/index?t='+new Date().getTime(),{size:10,name:$("#name").val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/parameter/index?t='+new Date().getTime(),{size:size,page:page_index, name:$("#name").val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += `<tr>
            ...
            <td>${v.author}</td>
            <td>
                <a class="btn btn-info btn-sm" href="/admin/parameter/update.html?id=${v.id}">修改</a>
				<a class="btn btn-info btn-sm" href="javascript:void(0)" onclick="del('${v.id}')">删除</a>
				<a class="btn btn-info btn-sm" href="/admin/parameterValue/index.html?parameterId=${v.id}">可选项管理</a>
			</td>
       </tr>` ;
        $list.append($(html));
    });
...
function del(id){
    ...
    $.ajax({
        url: "/api/parameter/delete?id=" + id,
        type: 'DELETE',
