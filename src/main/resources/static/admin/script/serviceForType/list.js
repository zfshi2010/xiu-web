...
var pageaction = function(){
    $.get('/api/serviceForType/index?t='+new Date().getTime(),{size:10,username:$("#username").val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/serviceForType/index?t='+new Date().getTime(),{size:size,page:page_index, username:$("#username").val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += `<tr>
            ...
            <td>${v.createTime}</td>
            <td>
				<a class="btn btn-info btn-sm" href="/admin/serviceForType/update.html?id=${v.id}">修改</a>
				<a class="btn btn-info btn-sm" href="javascript:void(0)" onclick="del('${v.id}')">删除</a>
				<a class="btn btn-info btn-sm" ${v.isMenu? '' : 'style="display:none"'} href="/admin/serviceForTypeMessage/index.html?serviceForTypeId=${v.id}">信息管理</a>
			</td>
       </tr>` ;
        $list.append($(html));
    });
...
function del(id){
    ...
    $.ajax({
        url: "/api/serviceForType/delete?id=" + id,
        type: 'DELETE',
        success: function(data) {
            if (data.status == 'success') {
                alert("删除成功");
                window.self.location = '/admin/serviceForType/index.html';
            } else {
                alert(data.message);
            }
        }
