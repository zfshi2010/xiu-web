...
var pageaction = function(){
    $.get('/api/measurementTask/index?t='+new Date().getTime(),{size:10,name:$("#name").val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/measurementTask/index?t='+new Date().getTime(),{size:size,page:page_index, name:$("#name").val()
        },function(data){
...
function del(id){
    ...
    $.ajax({
        url: "/api/measurementTask/delete?id=" + id,
        type: 'DELETE',
        success: function(data) {
            if (data.status == 'success') {
                alert("删除成功");
                window.self.location = '/admin/measurementTask/index.html';
            } else {
                alert(data.message);
            }
        }
