...
var pageaction = function(){
    $.get('/api/serviceForBrandProject/index?t='+new Date().getTime(),{size:10,serviceForBrandId:$("#serviceForBrandId").val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/serviceForBrandProject/index?t='+new Date().getTime(),{size:size,page:page_index, serviceForBrandId:$("#serviceForBrandId").val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += `<tr>
            ...
            <td>${v.createTime}</td>
            <td>
				<a class="btn btn-info btn-sm" href="/admin/serviceForBrandProject/update.html?id=${v.id}">修改</a>
...
function del(id){
    ...
    $.ajax({
        url: "/api/serviceForBrandProject/delete?id=" + id,
        type: 'DELETE',
        success: function(data) {
            if (data.status == 'success') {
                alert("删除成功");
                window.self.location = '/admin/serviceForBrandProject/index.html?serviceForBrandId=' + $('#serviceForBrandId').val();
            } else {
                alert(data.message);
            }
        }
