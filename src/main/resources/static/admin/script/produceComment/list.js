...
var pageaction = function(){
    $.get('/api/produceComment/index?t='+new Date().getTime(),{size:10,produceId:$("#produceId").val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/produceComment/index?t='+new Date().getTime(),{size:size,page:page_index, produceId:$("#produceId").val()
        },function(data){
...
function del(id){
    ...
    $.ajax({
        url: "/api/produceComment/delete?id=" + id,
        type: 'DELETE',
        success: function(data) {
            if (data.status == 'success') {
                alert("删除成功");
                window.self.location = '/admin/produceComment/index.html?produceId=' + $('#produceId').val();
            } else {
                alert(data.message);
            }
        }
