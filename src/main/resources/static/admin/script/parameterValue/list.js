...
var pageaction = function(){
    $.get('/api/parameterValue/index?t='+new Date().getTime(),{size:10,parameterId:$("#parameterId").val(),value:$('#value').val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/parameterValue/index?t='+new Date().getTime(),{size:size,page:page_index, parameterId:$("#parameterId").val(),value:$('#value').val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += `<tr>
            ...
            <td>${v.createTime}</td>
            <td>
				<a class="btn btn-info btn-sm" href="/admin/parameterValue/update.html?id=${v.id}">修改</a>
...
function del(id){
    ...
    $.ajax({
        url: "/api/parameterValue/delete?id=" + id,
        type: 'DELETE',
