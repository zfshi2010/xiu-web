...
var pageaction = function(){
    $.get('/api/measurementTaskType/index?t='+new Date().getTime(),{size:10,measurementTaskId:$("#measurementTaskId").val(),name:$('#name').val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/measurementTaskType/index?t='+new Date().getTime(),{size:size,page:page_index, measurementTaskId:$("#measurementTaskId").val(),name:$('#name').val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += `<tr>
            ...
            <td>${v.createTime}</td>
            <td>
				<a class="btn btn-info btn-sm" href="/admin/measurementTaskType/update.html?id=${v.id}">修改</a>
				<a class="btn btn-info btn-sm" href="javascript:void(0)" onclick="del('${v.id}')">删除</a>
				<a class="btn btn-info btn-sm" href="/admin/texture/index.html?measurementTaskTypeId=${v.id}">材质管理</a>
				<a class="btn btn-info btn-sm" href="/admin/measurementTaskTypeParameter/index.html?measurementTaskTypeId=${v.id}">属性管理</a>
			</td>
       </tr>` ;
        $list.append($(html));
    });
...
function del(id){
    ...
    $.ajax({
        url: "/api/measurementTaskType/delete?id=" + id,
        type: 'DELETE',
