...
var pageaction = function(){
    $.get('/api/productField/index?t='+new Date().getTime(),{size:10,username:$("#username").val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/productField/index?t='+new Date().getTime(),{size:size,page:page_index, username:$("#username").val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += `<tr>
            ...
            <td>${v.createTime}</td>
            <td>
				<a class="btn btn-info btn-sm" href="/admin/productField/update.html?id=${v.id}">修改</a>
				<a class="btn btn-info btn-sm" href="javascript:void(0)" onclick="del('${v.id}')">删除</a>
				<a class="btn btn-info btn-sm" href="/admin/productBrand/index.html?productFieldId=${v.id}">产品品牌管理</a>
			</td>
       </tr>` ;
        $list.append($(html));
    });
...
function del(id){
    ...
    $.ajax({
        url: "/api/productField/delete?id=" + id,
        type: 'DELETE',
        success: function(data) {
            if (data.status == 'success') {
                alert("删除成功");
                window.self.location = '/admin/productField/index.html';
            } else {
                alert(data.message);
            }
        }
