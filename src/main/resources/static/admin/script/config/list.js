...
var pageaction = function(){
    $.get('/api/message/index?t='+new Date().getTime(),{size:10,username:$("#username").val()},function(data){
...
var pageselectCallback = function(page_index, jq, size){
    ...
    if(currentPageData!=null){
        ...
        $.get('/api/message/index?t='+new Date().getTime(),{size:size,page:page_index, username:$("#username").val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        var html = "" ;
        html += `<tr>
            <td><img src="${v.smallImg}" width="120px"></td>
            <td>${v.title}</td>
            <td>${v.author}</td>
            <td>${v.createTime}</td>
            <td>
